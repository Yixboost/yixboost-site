...
//TFM version
define('VERSION', '2.6 (Yixboost Edition)');

//Application Title
define('APP_TITLE', 'Yixboost Files');
...
$json_file = "/volume1/web/yixboost/login/hotspot/logins.json";
...
if (file_exists($json_file)) {
    ...
    if (is_array($users_data)) {
        ...
        foreach ($users_data as $username => $user_details) {
            if (isset($user_details['files_acces']) && $user_details['files_acces'] == 1) {
                $auth_users[$username] = $user_details['hashed_password'];
            }
        }

        session_start();
        $_SESSION['auth_users'] = $auth_users;

    } else {
        echo "Fout: Kon de JSON-inhoud niet decoderen.";
    }
} else {
    echo "Fout: Het JSON-bestand bestaat niet op het pad: $json_file";
}
...
$favicon_path = 'http://naslambers.synology.me/yixboost/assets/images/logo-yixboost.png';
...
$external = array(
    ...
    'css-font-awesome' => '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" crossorigin="anonymous"><link rel="stylesheet" href="/yixboost/assets/font-awesome-6-pro-main/css/all.css" crossorigin="anonymous">',
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if (function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']]) && verifyToken($_POST['token'])) {
                ...
                fm_set_msg(lng('You are logged in. Welcome, ' . ucfirst($username)));
                fm_redirect(FM_SELF_URL);
            } else {
                unset($_SESSION[FM_SESSION_ID]['logged']);
                        ...
                        <div class="card fat" data-bs-theme="<?php echo FM_THEME; ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="mb-3">
                                        <div class="brand">
                                            <img src="https://yixgames.nl.eu.org/yixboost/assets/images/logo-yixboost.png">
                                        </div>
        ...
        <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
            <h6 class="card-header d-flex justify-content-between">
                ...
                <div class="card-body d-flex align-items-center">
                    <img src="https://yixgames.nl.eu.org/yixboost/portal/files/get_acc_image.php?usr=<?php echo $_SESSION[FM_SESSION_ID]['logged']; ?>" class="rounded-circle me-3" alt="User Image" width="100">
                    <div>
                        <h5 class="card-title mb-1"><?php echo ucfirst($_SESSION[FM_SESSION_ID]['logged']); ?></h5>
                        <p class="card-text mb-2"><?php echo fm_enc(APP_TITLE) ?> User</p>
                        <a href='/yixboost/newimage.php' target='_blank'><button class="btn btn-outline-secondary me-2"><i class="fas fa-camera"></i> <?php echo lng('Edit Profile Picture') ?></button></a>
                        <a href="?logout=1"><button class="btn btn-outline-danger"><i class="fas fa-sign-out-alt"></i> <?php echo lng('Logout') ?></button></a>
                    </div>
                </div>
            </div>
            <div class="card-body">
...
<div class="col-md-8 offset-md-2 pt-3">
    <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
        <h6 class="card-header d-flex justify-content-between">
            <span><i class="fa fa-server"></i> <?php echo lng('Server Info') ?></span>
            <a href="?p=<?php echo FM_PATH ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Cancel') ?></a>
        </h6>
        <div class="card-body">
            <h5 class="mb-3"><i class="fa fa-info-circle"></i> <?php echo lng('Details for Server') ?></h5>
            <ul class="list-group">
                <li class="list-group-item"><strong><?php echo lng('IP Address') ?>:</strong> <?php echo $_SERVER['SERVER_ADDR']; ?></li>
                <li class="list-group-item"><strong><?php echo lng('Hostname') ?>:</strong> <?php echo gethostname(); ?></li>
                <li class="list-group-item"><strong><?php echo lng('Operating System') ?>:</strong> <?php echo php_uname('s') . " " . php_uname('r'); ?></li>
                <li class="list-group-item"><strong><?php echo lng('Uptime') ?>:</strong> 
                    <?php
                        $uptime = shell_exec('uptime -p');
                        echo $uptime ? $uptime : lng('Uptime unavailable');
                    ?>
                </li>
                <li class="list-group-item"><strong><?php echo lng('CPU Load') ?>:</strong> 
                    <?php
                        $cpu_load = sys_getloadavg();
                        echo is_array($cpu_load) ? implode(', ', $cpu_load) : lng('Unavailable');
                    ?>
                </li>
                <li class="list-group-item"><strong><?php echo lng('Memory Usage') ?>:</strong>
                    <?php
                        $memory = shell_exec("free -h | grep Mem | awk '{print $3\" / \"$2}'");
                        echo $memory ? $memory : lng('Unavailable');
                    ?>
                </li>
                <li class="list-group-item"><strong><?php echo lng('Disk Usage') ?>:</strong> 
                    <?php
                        $disk = shell_exec("df -h / | tail -1 | awk '{print $3\" / \"$2\" (\"$5\")\"}'");
                        echo $disk ? $disk : lng('Unavailable');
                    ?>
                </li>
                <li class="list-group-item"><strong><?php echo lng('Active Processes') ?>:</strong>
                    <?php
                        $processes = shell_exec("ps -e | wc -l");
                        echo $processes ? trim($processes) . ' ' . lng('processes') : lng('Unavailable');
                    ?>
                </li>
                <li class="list-group-item"><strong><?php echo lng('PHP Version') ?>:</strong> <?php echo phpversion(); ?></li>
                <li class="list-group-item"><strong><?php echo lng('Web Server') ?>:</strong> <?php echo $_SERVER['SERVER_SOFTWARE']; ?></li>
            </ul>
        </div>
    </div>

<!-- PHP Info Section -->
<div class="card mt-4" style="border-left: 5px solid #777bb4;">
    <div class="card-header text-white" style="background-color: #4F5B93;">
        <h5 class="d-flex align-items-center">
            <i class="fa-brands fa-php me-2" style="font-size: 2rem; color: #8993BE;"></i> 
            <?php echo lng('PHP Information'); ?> (<?php echo lng('Version') . ' ' . phpversion(); ?>)
        </h5>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    if ($phpLibraries) {
                        $librariesArray = explode("\n", trim($phpLibraries));
                        $chunkedLibraries = array_chunk($librariesArray, 5); 

                        foreach ($chunkedLibraries as $libraries) {
                            echo '<tr>';
                            foreach ($libraries as $library) {
                                echo '<td>' . htmlspecialchars($library) . '</td>';
                            }
                            while (count($libraries) < 5) {
                                echo '<td>&nbsp;</td>';
                                $libraries[] = ""; 
                            }
                            echo '</tr>';
                        ...
                        echo '<tr>';
                        echo '<td colspan="5">Unable to fetch PHP extensions.</td>';
                        echo '</tr>';
                    }
                    ?>
                </tbody>
...
<!-- Python Info Section -->
<div class="card mt-4" style="border-left: 5px solid #007BFF;">
    <div class="card-header text-white" style="background-color: #0069D9;">
        <h5 class="d-flex align-items-center">
            <i class="fab fa-python me-2" style="font-size: 2rem; color: #FFD43B;"></i> 
            <?php echo lng('Python Information'); ?> (<?php echo lng('Version') . ' ' . shell_exec('python3 --version'); ?>)
        </h5>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <!-- Python Version -->
                    <tr>
                        <td><?php echo lng('Python Version'); ?></td>
                        <td><?php echo shell_exec('python3 --version'); ?></td>
                    </tr>
                    <!-- Python Executable Path -->
                    <tr>
                        <td><?php echo lng('Python Executable Path'); ?></td>
                        <td><?php echo shell_exec('which python3'); ?></td>
                    </tr>
                    <!-- System Info -->
                    <tr>
                        <td><?php echo lng('Operating System'); ?></td>
                        <td><?php echo shell_exec('uname -a'); ?></td>
                    </tr>
                    <!-- Python memory and CPU usage -->
                    <tr>
                        <td><?php echo lng('Memory Usage'); ?></td>
                        <td><?php echo shell_exec('free -h'); ?></td>
                    </tr>
                    <tr>
                        <td><?php echo lng('CPU Usage'); ?></td>
                        <td><?php echo shell_exec('top -bn1 | grep "Cpu(s)"'); ?></td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        ...
                        <p>Author: PRAŚATH MANİ, edited by Yixboost NL</p>
...
<div class="col-md-8 offset-md-2 pt-3">
    <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
        <h6 class="card-header d-flex justify-content-between">
            <span><i class="fa fa-terminal"></i> <?php echo lng('PHP Shell Exec Terminal') ?></span>
            <a href="?p=<?php echo FM_PATH ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Cancel') ?></a>
        </h6>
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    <p><strong>Execute Commands in PHP Terminal</strong></p>
                    
                    <!-- Display previous output -->
                    <div class="terminal-output" style="background-color: black; color: green; padding: 10px; border-radius: 5px; height: 300px; overflow-y: auto; white-space: pre-wrap; font-family: monospace; display: flex; flex-direction: column-reverse;">
                        <?php
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['command']) && isset($_GET['p'])) {
    $command = escapeshellcmd($_POST['command']); // Sanitize command input
    $folder = $_GET['p']; // Get the folder path from the query string

    // Ensure the folder is valid and exists
    $fullPath = '/volume1/web/' . $folder;
    if (is_dir($fullPath)) {
        chdir($fullPath); // Change the current directory to the desired path
        ...
        if ($output) {
            ...
            echo 'Error: Command did not return any output or is not allowed.';
        }
    } else {
...
?>
                    ...
                    <form class="form-inline mt-3" method="POST" action="">
                        <div class="form-group mb-2">
                            <!-- Display the prompt at the start of input -->
                            <label for="terminalCommand" class="sr-only"><?php echo lng('Enter Command') ?></label>
                            <input type="text" class="form-control" id="terminalCommand" name="command" placeholder="> " required style="width: 100%; padding: 10px; font-family: monospace; background-color: black; color: green; border: none;">
                        </div>
                        <button type="submit" class="btn btn-primary btn-sm mt-2" style="display: none;"><?php echo lng('Execute') ?></button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    // Automatically scroll to the bottom of the terminal output when new content is added
    const terminalOutput = document.querySelector('.terminal-output');
    terminalOutput.scrollTop = terminalOutput.scrollHeight;

    // Keep the prompt at the top of the terminal, even after submitting commands
...
if ($is_onlineViewer) {
    if ($ext == 'pdf') {
        echo '<iframe src="' . fm_enc($file_url) . '" type="application/pdf" frameborder="no" style="width:100%;min-height:460px"></iframe>';
    ...
    // Audio content met WaveSurfer.js
    echo '<div id="waveform" style="margin-bottom: 10px;"></div>';
    echo '
    <div id="controls" style="display: flex; gap: 10px; align-items: center;">
        <button id="playPauseBtn" style="padding: 5px 10px;">Play/Pause</button>
        <button id="stopBtn" style="padding: 5px 10px;">Stop</button>
        <label>Volume:</label>
        <input type="range" id="volumeSlider" min="0" max="1" step="0.1" value="0.5" style="width: 100px;">
        <label>Seek:</label>
        <input type="range" id="seekSlider" min="0" max="100" step="1" value="0" style="width: 200px;">
    </div>
    <script src="https://unpkg.com/wavesurfer.js"></script>
    <script>
        // Initialiseer WaveSurfer met de gewenste instellingen
        ...
        wavesurfer.load("' . fm_enc($file_url) . '");
        ...
        // Update seekSlider tijdens het afspelen
        wavesurfer.on("audioprocess", () => {
            const currentProgress = Math.round(wavesurfer.getCurrentTime() / wavesurfer.getDuration() * 100);
            document.getElementById("seekSlider").value = currentProgress;
        });

        // Reset seekSlider na het stoppen
    ...
    if ($ext === 'md') {
        $content = '<div id="markdown-renderer">' . fm_enc($content) . '</div>';
        $content .= '
            <script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script>
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    const renderer = document.getElementById("markdown-renderer");
                    renderer.innerHTML = marked.parse(renderer.textContent);
                });
            </script>';
} elseif ($ext === 'json') {
    $json_data = $content; 
    $content = '<div id="jsoneditor"></div>';
    $content .= '
    ...
    <script>
        ...
        // Zet de PHP JSON data om naar een JavaScript-object
...
function fm_get_file_icon_class($path)
{
    ...
    switch ($ext) {
        ...
        case 'sql':
        case 'js':
            $img = 'fa-brands fa-js'; // Font Awesome 6 JavaScript icon
            break;
        case 'ts':
        case 'jsx':
        case 'tsx':
            $img = 'fa-brands fa-js'; // Font Awesome 6 JavaScript icon for TypeScript and JSX
            break;
        case 'hbs':
        case 'json':
        case 'sh':
        ...
        case 'tpl':
        case 'md':
            $img = 'fa fa-file-code-o'; // Keep the file code icon for these
            break;
        case 'gitignore':
            $img = 'fa-brands fa-git-alt';
            break;
        case 'c':
        case 'cpp':
        case 'cs':
            $img = 'fa fa-file-code-o'; // You can leave this for C/C++/C# code files
            break;
        case 'py':
            $img = 'fa-brands fa-python'; // Font Awesome 6 Python icon
            break;
        case 'rs':
            $img = 'fa fa-file-code-o'; // Rust files can use the general file code icon
            break;
        case 'map':
        case 'lock':
        case 'dtd':
        case 'ps1':
            $img = 'fa fa-file-code-o'; // Use the file code icon for these as well
            break;
        case 'txt':
        case 'ini':
        case 'conf':
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo FM_THEME; ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
                        ...
                        <li class="nav-item">
                            <a title="<?php echo lng('Terminal') ?>" class="nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;terminal=1"><i class="fa-solid fa-rectangle-terminal"></i></a>
                        </li>
                    <?php endif; ?>
<?php if (FM_USE_AUTH): ?>
    <li class="nav-item avatar dropdown">
        <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-bs-toggle="dropdown" aria-expanded="false">
            <!-- Correct way to use PHP variable inside src -->
            <img id="profile-picture" src="https://yixgames.nl.eu.org/yixboost/portal/files/get_acc_image.php?usr=<?php echo $_SESSION[FM_SESSION_ID]['logged']; ?>" alt="Profile Picture" width="25px" height="auto" style='border-radius: 50%;'>
        </a>
        <div class="dropdown-menu text-small shadow" aria-labelledby="navbarDropdownMenuLink-5" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            <a title="<?php echo lng('Server Info') ?>" class="dropdown-item nav-link" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;server=2"><i class="fa-solid fa-server" aria-hidden="true"></i> <?php echo lng('Server Info') ?></a>
    ...
    <head>
        ...
        <style>
            ...
            *::after {
            ...
            html[data-bs-theme="dark"] #main-table span.badge {
...
editor.setTheme(theme === "light" ? "ace/theme/chrome" : "ace/theme/twilight");
